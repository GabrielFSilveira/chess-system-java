...
public class ChessMatch {
	...
	//metodo aux para instanciar a peça especifica
	...
	private Piece makeMove(Position source, Position target) {
		ChessPiece p = (ChessPiece)board.removePiece(source); // retirei a peça que tava na posição de origem
		p.increaseMoveCount();
		...
		// #especialmove roque curto
		if (p instanceof King && target.getColumn() == source.getColumn() + 2){
			Position sourceT = new Position(source.getRow(), source.getColumn() + 3);
			Position targetT = new Position(source.getRow(), source.getColumn() + 1);
			ChessPiece rook = (ChessPiece)board.removePiece(sourceT); //retira a torre nde origem
			board.placePiece(rook, targetT); //coloca na posição de destino
			rook.increaseMoveCount(); //incrementa a quantidade de movimentos
		}

		// #especialmove roque longo(grande)
		if (p instanceof King && target.getColumn() == source.getColumn() - 2){
		...
		if (p instanceof Pawn) {
			//se o peão andou na diagonal e não capturou peça, foi o en passant
